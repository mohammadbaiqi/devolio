...
export default function DetailPage() {
  ...
  if (loading) {
    return <div className="px-6 py-24 text-white">Loading project...</div>;
  }

  if (!project) {
    return <div className="px-6 py-24 text-white">Project not found.</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12 text-white md:px-12 md:py-24">
     ...
     <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      ...
      <p className="justify mb-8 max-w-6xl text-lg text-white/70">{project.overview}</p>
      ...
      <div className="mb-8 grid gap-4 rounded-3xl border border-white/10 bg-white/5 p-6 md:grid-cols-2">
        <div>
          <p className="text-sm uppercase tracking-[0.2em] text-white/40">Year</p>
          <p className="mt-2 text-lg">{project.info?.year ?? "—"}</p>
        </div>
        <div>
          <p className="text-sm uppercase tracking-[0.2em] text-white/40">Team</p>
          <p className="mt-2 text-lg">{project.info?.team ?? "—"}</p>
        </div>
        <div>
          <p className="text-sm uppercase tracking-[0.2em] text-white/40">Role</p>
          <p className="mt-2 text-lg">{project.info?.role ?? "—"}</p>
        </div>
        <div>
          <p className="text-sm uppercase tracking-[0.2em] text-white/40">Duration</p>
    ...
    <div className="space-y-6">
         <h2 className="mb-4 text-2xl font-semibold">What I Build   </h2>
